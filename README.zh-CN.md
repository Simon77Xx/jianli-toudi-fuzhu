...
1. 上传完整的 LaTeX 项目 ZIP。ZIP 中应包含主 `.tex`、`.cls`、图片、字体和其他编译依赖。如果模板同时提供中英文入口（例如 `resume-zh.tex` 和 `resume-en.tex`），工具会优先选择文件名或 `documentclass` 中标记为 `zh`、`cn`、`chinese` 的中文入口；仍无法判断时才会要求你删除或重命名多余入口。
...
- 结果会同时显示“修改前”和“修改后”两组分数，并计算变化值（修改后 - 修改前）。修改前分数由一次独立模型调用生成，修改后分数再按定制简历重新评估，便于判断本次微调是否带来提升。
