...
from .latex import compact_to_two_pages, copy_compiled_pdf, create_tailored_project, make_overleaf_zip
from .llm import ModelServiceError, require_configured_gateway
from .models import Evidence, FinalAnalysis, InitialAnalysis, JobContext, Question
...
class ResumePipeline:
    ...
    def finalize(self, session_id: str) -> dict[str, object]:
        ...
        report_file.write_text(
            self._render_report(context, analysis, compile_result, actions, rejected), encoding="utf-8"
        )
        return {
            "analysis": analysis,
    ...
    @staticmethod
    def _apply_compile_guard(analysis: FinalAnalysis, compile_result) -> tuple[FinalAnalysis, list[str]]:
        warnings: list[str] = []
    ...
    def _render_report(self, context: JobContext, analysis: FinalAnalysis, compile_result, actions: list[str], rejected: list[str]) -> str:
        lines = [
            "# JD 定制简历报告",
            "",
            "## HR / AI 初筛匹配摘要",
